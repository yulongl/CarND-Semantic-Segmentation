...
def layers(vgg_layer3_out, vgg_layer4_out, vgg_layer7_out, num_classes):
    ...
    print(conv_1x1_7)
    output_4 = tf.layers.conv2d_transpose(conv_1x1_7, num_classes, 4, 2, padding='same',
                                          kernel_regularizer=tf.contrib.layers.l2_regularizer(1e-3),
                                   kernel_initializer= tf.random_normal_initializer(stddev=1e-3))
    print(output_4)
    conv_1x1_4 = tf.layers.conv2d(vgg_layer4_out, num_classes, 1, padding='same',
                                  kernel_regularizer=tf.contrib.layers.l2_regularizer(1e-3),
                                   kernel_initializer= tf.random_normal_initializer(stddev=1e-3))
    print(conv_1x1_4)
    sum_4 = tf.add(output_4, conv_1x1_4)
    output_3 = tf.layers.conv2d_transpose(sum_4, num_classes, 4, strides=(2, 2), padding='same',
                                          kernel_regularizer=tf.contrib.layers.l2_regularizer(1e-3),
                                   kernel_initializer= tf.random_normal_initializer(stddev=1e-3))
    conv_1x1_3 = tf.layers.conv2d(vgg_layer3_out, num_classes, 1, padding='same',
                                  kernel_regularizer=tf.contrib.layers.l2_regularizer(1e-3),
                                   kernel_initializer= tf.random_normal_initializer(stddev=1e-3))
    print(conv_1x1_4)
    sum_3 = tf.add(output_3, conv_1x1_3)
    output_last = tf.layers.conv2d_transpose(sum_3, num_classes, 16, strides=(8, 8), padding='same',
                                             kernel_regularizer=tf.contrib.layers.l2_regularizer(1e-3),
                                   kernel_initializer= tf.random_normal_initializer(stddev=1e-3))
    return output_last
...
def optimize(nn_last_layer, correct_label, learning_rate, num_classes):
    ...
    return logits, training_operation, total_loss
...
def train_nn(sess, epochs, batch_size, get_batches_fn, train_op, cross_entropy_loss, input_image,
    ...
    for epoch in range(epochs):
        ...
        loss_ave = sum(loss_data) / float(len(loss_data))
        print(loss_ave)
        print()
    print('Complete')
